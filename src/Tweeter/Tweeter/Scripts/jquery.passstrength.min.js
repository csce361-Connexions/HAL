...
"a".charCodeAt(0);var g=void 0;g=null;if(o){g=k.charAt(o-1);if(typeof p[g]!="undefined")g=p[g]}if(g!=null&&g.match(/^[a-zA-Z]$/)){k=void 0;k=g.toLowerCase().charCodeAt(0)-"a".charCodeAt(0);g=A[l+26*k]}else g=(o?y:z)[l];if(f>=26)g*=26/f;f=g}else f=1/f;d+=Math.max(-7,Math.log(f)/0.6931471805599453)}return-1*d},C=function(b){var c=0,d=0;d=B(b);if(i.length&&i[0]instanceof RegExp)$.each(i,function(j,f){c+=b.search(f)!=-1});else for(e=0;e<i.length;e++){if(d<i[e])break;c=e+1}return[d,c]},x=function(){var b=
...
var b=Math.round((125-3*i.length)/i.length);for(e=0;e<i.length;e++){var c=h("<span>").css("height","3px").css("width",b+"px").css("margin-right","3px").css("max-height","3px").css("font-size","1px").css("float","left");t.push(c);m.append(c)}q.keypress(x);q.keyup(x);q.trigger("keyup")})}})($);
